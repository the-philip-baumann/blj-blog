...
<div class="wrapper">
    ...
    <?php foreach($allPostsTable as $row): ?>
        ...
        ?>
            <input name="id" id="id" value="<?= htmlspecialchars($row['id'], ENT_QUOTES, "UTF-8"); ?> "></input>
            <h3><?= htmlspecialchars($row['created_by'], ENT_QUOTES, "UTF-8"); ?></h3>
            <h4><?= htmlspecialchars($row['post_title'], ENT_QUOTES, "UTF-8"); ?></h4>
            ...
            <form action="index.php?page=comment" method="POST">
                <div class="form-actions">
                    <input class="btn btn-primary" type="submit" value="comment" name="btn-comment">
                </div>
    ...
    <input class="form-control" type="hidden" id="id" name="id" name="id" value="<?=htmlspecialchars($row['id']);?>" >
    <input class="form-control" type="hidden" id="thumbsUP" name="thumbsUP" value="<?=htmlspecialchars($row['up_votes']);?>">
    <input class="form-control" type="hidden" id="thumbsDown" name="thumbsDown" value="<?htmlspecialchars($row['down_votes']);?>">

    <p>
        <?=htmlspecialchars($row['up_votes']);?>
        <?=htmlspecialchars($row['down_votes']);?>
    </p>
    </form>
        </div>
    <?php endforeach; ?>
